...
function VehicleDetail({vehicle,setVehicles,suppliers,customers,accounts,labourers,addTxn,expenseCategories,transactions,onBack}){
  ...
  const confirmImportSales=()=>{
    ...
    const newSales=valid.map(r=>({id:genId(),customerId:r.customerId,customerName:(customers.find(c=>c.id===r.customerId)?.name||r.custName),date:r.date,weight:n(r.weight),rate:n(r.rate),totalAmount:n(r.weight)*n(r.rate),receiptNo:`RCP-${genId()}`,notes:r.notes||"Imported",receipts:[]}));
    mut(v=>({...v,sales:[...v.sales,...newSales]}));
